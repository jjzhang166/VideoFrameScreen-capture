...
void Widget::playAndSaveVideo()
{
    ...
    else
    {
        ...
        if ( selectedItem )
        {
            ...
            savePath = savePathRoot +"\\" + videoName + "\\" + videoName;
...
void Widget::saveVideoPicture()
{
    ...
    else
    {
        ...
        qint64 startTime = (ui->lineEditBegin->text().toDouble()/1000.0)*thread->getFFmpeg()->getTotalTime();
        qint64 endTime = (ui->lineEditEnd->text().toDouble()/1000.0)*thread->getFFmpeg()->getTotalTime();

        //得到非上下课的起始时间和终止时间的long值
        //得到过度上下课的起始时间和终止时间的long值
        ...
        if(ffmpegNotShow->Init()){
            ...
            emit sendSavePath(savePath);
            t->start();
//            qDebug() << "已经执行了thread的start方法，但是卵用都没有";
        }
        else
        {
            QMessageBox::warning(this,"Warning","ffmpeg init failed",QMessageBox::Ok);
